...
clear;
format long;
N = 80;                                  %** Number of Transmitted symbols   **%
M = 8;
...
%     h = ones(2,1);                         %** Ideal channel  (AWGN only)                **%
    ...
    for k=1:N
      ...
      if (x(3)==0)
          if (x(1) == 0 && x(2) == 0)   
              rd1 = h(1)*s1 + sqrt(a)*crandn(1,1);  %** received signal point for 00              **%
          elseif (x(1) == 0 && x(2) == 1)
              rd1 = h(1)*s2 + sqrt(a)*crandn(1,1);  %** received signal point for 01              **%
          elseif (x(1) == 1 && x(2) == 0)
          ...
          if (x(1) == 0 && x(2) == 0)   
              rd1 = h(2)*s5 + sqrt(a)*crandn(1,1);  %** received signal point for 00              **%
          elseif (x(1) == 0 && x(2) == 1)
              rd1 = h(2)*s6 + sqrt(a)*crandn(1,1);  %** received signal point for 01              **%
          elseif (x(1) == 1 && x(2) == 0)
              rd1 = h(2)*s7 + sqrt(a)*crandn(1,2);  %** received signal point for 10              **%
          else
              rd1 = h(2)*s8 + sqrt(a)*crandn(1,2);  %** received signal point for 11              **%
          end
      end
      ...
      met(5) = sum(abs(rd1 - h(2)*s5).^2);      %  0 0 %
      met(6) = sum(abs(rd1 - h(2)*s6).^2);      %  0 1 %
      met(7) = sum(abs(rd1 - h(2)*s7).^2);      %  1 0 %
      met(8) = sum(abs(rd1 - h(2)*s8).^2);      %  1 1 %
      [med,nd] = min(met);
